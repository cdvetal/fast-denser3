...
def load_train(wnids, dataset_path, shape):
    ...
    for idx, wnid in enumerate(wnids):
        for j in range(500):
            im = Image.open(
                f'{dataset_path}/train/{wnid}/images/{wnid}_{j}.JPEG'
            ).convert('RGB')
...
def load_test(wnids, dataset_path, shape):
    ...
    with open(f'{dataset_path}/val/val_annotations.txt') as ann:
        annotations = ann.readlines()

    for i, line in enumerate([s.strip() for s in annotations]):
        name, wnid = line.split('\t')[:2]

        im = Image.open(f'{dataset_path}/val/images/{name}').convert('RGB')
...
def load_tiny_imagenet(dataset_path, shape):
    ...
    wnids = [
        x.strip() for x in open('%s/wnids.txt' % dataset_path).readlines()
    ]
