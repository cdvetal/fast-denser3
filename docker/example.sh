...
docker run -it -v$PWD:/code fast-denser3 /bin/bash -c "cd /code/f-denser && $LD_PRELOAD DEBUG=True python3 -m fast_denser.engine -d mnist -c /code/example/config.json -r 1 -g /code/example/cnn.grammar"
